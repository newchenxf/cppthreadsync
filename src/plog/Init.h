...
#include <plog/Formatters/TxtFormatterSimple.h>
...
namespace plog
{
    ...
    //定制版本，不记录tid, function, 用于pingback
    template<int instanceId>
    inline Logger<instanceId>& initSimple(Severity maxSeverity, const util::nchar* fileName, size_t maxFileSize = 0, int maxFiles = 0)
    {
        return isCsv(fileName) ? init<CsvFormatter, instanceId>(maxSeverity, fileName, maxFileSize, maxFiles) : init<TxtFormatterSimple, instanceId>(maxSeverity, fileName, maxFileSize, maxFiles);
    }
    //////////////////////////////////////////////////////////////////////////
